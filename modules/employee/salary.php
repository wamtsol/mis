...
$employee = dofetch( doquery( "select * from employee where id = '".slash( $_GET[ "id" ] )."'", $dblink ) );
$sql = "SELECT id, employee_id, CONCAT(LAST_DAY(CONCAT(year,'-',month+1,'-01')),' 00:00:00') as dt, concat( MONTHNAME(LAST_DAY(CONCAT(year,'-',month+1,'-01'))), ' ', year, ' salary') as details, 0 as debit, amount as credit FROM employee_salary where status=1 and employee_id = '".$employee[ "id" ]."' union select a.id, employee_id, a.datetime_added as dt, concat(a.details, if(a.details='','', ' - '),'Paid by ', b.title), amount as debit, 0 as credit from employee_salary_payment a left join account b on a.account_id = b.id where a.status=1 and employee_id = '".$employee[ "id" ]."' order by dt desc";

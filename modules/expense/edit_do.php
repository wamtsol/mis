...
if(isset($_POST["expense_edit"])){
	...
	if($err==""){
		$sql="Update expense set `project_id`='".slash($project_id)."',`datetime_added`='".slash(datetime_dbconvert($datetime_added))."',`expense_category_id`='".slash($expense_category_id)."',`account_id`='".slash($account_id)."',`details`='".slash($details)."',`amount`='".slash($amount)."',`cheque_number`='".slash($cheque_number)."'"." where id='".$id."'";
		doquery($sql,$dblink);

...
<form class="form-horizontal form-horizontal-left" role="form" action="expense_manage.php?tab=edit" method="post" enctype="multipart/form-data" name="frmAdd">
    	...
    	<div class="row">
            ...
            <div class="col-sm-10">
                <select name="employee_id" title="Choose Option">
                    ...
                    <?php
                    $res=doquery("select * from admin where status=1 order by id", $dblink);
                    if(numrows($res)>0){
                        while($rec=dofetch($res)){
                        ?>
                        <option value="<?php echo $rec["id"]?>"<?php echo($employee_id==$rec["id"])?"selected":"";?>><?php echo unslash($rec["username"]); ?></option>
                     	<?php			
                        }			
                    }
                    ?>
    ...
    <div class="form-group">
    	<div class="row">
            ...
            <div class="col-sm-10">
                <select name="category_id" title="Choose Option">
                    <option value="0">Select Category</option>
                    <?php
                    $res=doquery("select * from category where status=1 order by title", $dblink);
                    if(numrows($res)>0){
                        while($rec=dofetch($res)){
                        ?>

...
if(isset($_POST["project_payment_add"])){
	...
	if($err==""){
		$sql="INSERT INTO project_payment (project_id, datetime_added, amount, sales_tax, gst_withheld, invoice_amount, account_id, details, exempt_tax) VALUES ('".slash($project_id)."','".slash(datetime_dbconvert($datetime_added))."','".slash($amount)."','".slash($sales_tax)."','".slash($gst_withheld)."','".slash($invoice_amount)."','".slash($account_id)."','".slash($details)."','".slash($exempt_tax)."')";
		doquery($sql,$dblink);

    	...
    	<div class="row">
            ...
            <div class="col-sm-10">
                <select name="account_id" title="Choose Option">
                    ...
                    <?php
                    $res=doquery("select * from account where status=1".(!empty($_SESSION["transaction"]["list"]["project_id"])?" and id in (select account_id from project_2_account where project_id = '".$_SESSION["transaction"]["list"]["project_id"]."')":"")." order by title", $dblink);
                    if(numrows($res)>0){
                        while($rec=dofetch($res)){
                        ?>

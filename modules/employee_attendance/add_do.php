...
if(isset($_POST["employee_attendance_save"])){
	...
	if( numrows( doquery( "select id from employee_daily_attendance where date='".$date."'", $dblink ) ) > 0 ) {
		header("Location: employee_attendance_manage.php?tab=add&err=".url_encode( "Attendance already taken for this day." ));
	}
	else{
